...
class ChatService:
    ...
    async def _process_with_ai(
        ...
        try:
            ...
            if self._has_booking_intent(message) and self._is_data_complete(session.prospect_data):
    ...
    async def _handle_booking_intent(self, session: ConversationSession) -> str:
        ...
        if email_sent:
            ...
            # inventory_service.reserve_unit(available_unit.unit_id)
            # session.prospect_data.unit_id = available_unit.unit_id
           # Email failed—inform the user explicitly and provide the leasing office number
            return (
               f"Your tour is confirmed for {tour_date} at {tour_time}, "
                "but I ran into an error sending the email confirmation. "
                f"Please call our leasing office at {email_service.leasing_office_phone} "
                "to make sure you have your booking details."
           )
